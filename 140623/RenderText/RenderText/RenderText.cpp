...
void Render(int timeDelta)
{
	...
	//	static int ntimeDelay = 0;
	//	float nfps = 0.f;
	//	string fps;
	//						
	//	if( ntimeDelay >= 1000 )
	//	{
	//		nfps = frameCnt / ntimeDelay;
	//		fps = format( "%f", nfps );
	//		frameCnt = ntimeDelay = 0;
	//	}
	//	else
	//	{
	//		frameCnt++;
...
//		Quaternion quat;
//		quat.SetRotationArc(Vector3(0,1,0), Vector3(1,0,0));  //첫번째 인자방향 기준으로 두번째 인자 방향으로 
//		Matrix44 qm = quat.GetMatrix();


		
		Matrix44 mat;
		mat.SetTranslate(Vector3(5, 0, -490));
