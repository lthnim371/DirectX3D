...
bool cShader::Create(const string &fileName, const string &technique)
{
	...
	if (FAILED(hr = D3DXCreateEffectFromFileA(
		...
		&m_effect, // 컴파일된 이펙트 파일이 저장될 버퍼  //m_effect : 에러나면 NULL
		&pErr // 컴파일 에러가 저장될 버퍼
		))) 
	{
	...
	m_hTechnique = m_effect->GetTechniqueByName( technique.c_str() );  //핸들 가져오기
...
void cShader::Begin()
{
	...
	m_effect->SetTechnique( m_hTechnique );  //가져온 핸들로 사용하겠다라는 의미
}
...
//정점마다 최종적인 행렬을 곱해줌(매 프레임마다 해줘야함)
