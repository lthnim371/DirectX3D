...
void cModel::Render()
{
	...
	for (int i=0; i < (int)m_boundingBox.size(); ++i)
		m_boundingBox[ i].Render( m_bone->GetPalette()[ i] * m_matTM);  //m_matTM로 인해 모델을 움직여도 있어도 따라 움직임(경계박스)
}
...
void cModel::CreateBoneBoundingBox(const string &modelName)
{
	...
	{
		boneInvers[ i] = rawMeshes->bones[ i].worldTm.Inverse();  //본을 로컬좌표로 변환하기 위한 준비
	...
	BOOST_FOREACH (const sRawMesh &mesh, rawMeshes->meshes)
	{
		BOOST_FOREACH (const sVertexWeight &weight, mesh.weights)
		{
			...
			for( int k=0; k < weight.size; ++k )
			{
				...
				const Vector3 pos = mesh.vertices[ vtxIdx] * boneInvers[ w->bone];  //로컬좌표로 변환
	...
	for (int i=0; i < boneCount; ++i)
	{
		...
		m_boundingBox[ i].SetTransform(rawMeshes->bones[ i].worldTm);  //월드TM넣음
	}
}
