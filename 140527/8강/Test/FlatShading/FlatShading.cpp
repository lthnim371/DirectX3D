	...
	const Matrix44 &tm, const Matrix44 &vpv)
{
	...
	for (unsigned int i=0; i < indices.size(); i+=3)
	{
		...
		diffuse = max(n.DotProduct(-light),0);

		Vector3 testH = light + camDir;
		testH.Normalize();

		specular = pow(max(light.DotProduct(testH), 0), 16);
		Rasterizer::Color testC(255,255,255,1);
		testC = testC * specular;
...
//		Rasterizer::Color color = c0 * diffuse;
		Rasterizer::Color color = c0 + testC + diffuse;
