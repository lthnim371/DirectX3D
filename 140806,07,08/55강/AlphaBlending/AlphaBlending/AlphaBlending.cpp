...
void Render(int timeDelta)
{
		...
		)))
	{
		...
		// 알파블렌딩 플래그를 On 한다.
		g_pDevice->SetTextureStageState( 0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE );  //D3DTA_TEXTURE 대신 D3DTA_DIFFUSE로 넣을 수 있었다...
		// 첫 번째 알파 인자를 텍스쳐에 있다고 알려주고,
		g_pDevice->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_SELECTARG1 );
		// 알파 연산 없이 첫 번째 알파 채널을 그대로 이용하겠다고 알린다.
		g_pDevice->SetRenderState( D3DRS_SRCBLEND, D3DBLEND_SRCALPHA );  //= src * alpha
		g_pDevice->SetRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );  //= des * (1-alpha)
...
bool InitVertexBuffer()
{
	...
	g_grid.GetTexture().Create( "../../media/grass_spring3.dds");  //텍스쳐 파일 불러온듯?
