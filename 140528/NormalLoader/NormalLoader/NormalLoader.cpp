...
vector<Vector3> g_normals;  //1번 넣음
...
//bool ReadModelFile( const string &fileName, vector<Vector3> &vertices, vector<int> &indices);  //원래것삭제
//2번 넣음
...
//4번 넣음
	...
	{
		float num1, num2, num3;
		vector<int> vertCount(numVertices, 0);  //카운트는 해당 버텍스가 호출되는 갯수대로 계속 더하여 나중에 나눈다(평균 구하기 위해)
		for (int i = 0; i < numNormal; i++)
		{
			...
			for (int k=0; k < 3; ++k)
			{
				const int vtxIdx = indices[ i*3 + k];  //버텍스의 인덱스를 가리키는 것
				normals[ vtxIdx] += n;  //각각의 페이스 버퍼 값을 
				++vertCount[ vtxIdx];
			}
		}
	...
	const Matrix44 &tm, const Matrix44 &vpv)
{
	...
	for (unsigned int i=0; i < indices.size(); i+=3)
	{
		...
		n1 = n1.MultiplyNormal(tm);  //회전만 해야 한다. 이동하게되면 원래의 노말벡터가 사라지고 이동한 곳의 노말로 됨
		Vector3 n2 = normals[ indices[ i+1]];
		n2 = n2.MultiplyNormal(tm);
		Vector3 n3 = normals[ indices[ i+2]];
		n3 = n3.MultiplyNormal(tm);
		
		
		if ((n1.DotProduct(camDir) > 0) &&
			(n2.DotProduct(camDir) > 0) &&
			(n3.DotProduct(camDir) > 0))
			continue;
		...
		
		{
			...
			const float a2 = max(n1.DotProduct(H), 0);
		...
		}
		{
			...
			const float a2 = max(n2.DotProduct(H), 0);
			Rasterizer::Color specular = lightColor * pow(a2, 16);
			color2 = ambient + diffuse + specular;
		}
		{
			Rasterizer::Color diffuse = c0 * max(0, n3.DotProduct(-lightDir));
			const float a2 = max(n3.DotProduct(H), 0);
			Rasterizer::Color specular = lightColor * pow(a2, 16);
			color3 = ambient + diffuse + specular;
		}

		Rasterizer::DrawTriangle(hdc, color1, p1.x, p1.y, color2, p2.x, p2.y, color3, p3.x, p3.y);
	}
}
