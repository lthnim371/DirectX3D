...
//이하 4개의 코드들은 해당 스테이지에 있는 이미지에 있는 정보들을 사용하겠다라는 의미이다.(디폴트값에 가깝다)
		g_pDevice->SetTextureStageState( 0, D3DTSS_COLOROP,   D3DTOP_SELECTARG1 );  //D3DTSS_COLOROP : 칼라연산  //D3DTOP_SELECTARG1을 사용하겠다 의미(내려오는 ARG2가 없기에 ARG1을 사용)
		g_pDevice->SetTextureStageState( 0, D3DTSS_COLORARG1, D3DTA_TEXTURE );  //D3DTOP_SELECTARG1에 0번 스테이지에 있는 텍스쳐를 선택하겠다 의미
		g_pDevice->SetTextureStageState( 0, D3DTSS_ALPHAOP,   D3DTOP_SELECTARG1 );  //최종적인 알파값은 마지막에 해주기 때문에 사실상 의미가 없다고 볼 수 있다.
		g_pDevice->SetTextureStageState( 0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE );  //최종적인 알파값은 마지막에 해주기 때문에 사실상 의미가 없다고 볼 수 있다.
//두번째 인자 플래그가 ㅇㅇㅇOP인 것들의 3번째 인자 플래그에 D3DTOP_SELECTARG1 또는 D3DTOP_SELECTARG2 등으로 설정하면 해당 arg의 정보로 사용하겠다라는 의미이다...그러면 즉, 연산은 의미없어지고 해당 arg 정보로 값이 결정된다.
		g_pDevice->SetTextureStageState( 1, D3DTSS_COLOROP, D3DTOP_MODULATE);  //D3DTOP_MODULATE : 칼라값 곱하기
		g_pDevice->SetTextureStageState( 1, D3DTSS_COLORARG1, D3DTA_TEXTURE);  //1번 스테이지의 arg1에 1번 텍스쳐를 선택
		g_pDevice->SetTextureStageState( 1, D3DTSS_COLORARG2, D3DTA_CURRENT);  //D3DTA_CURRENT : 위에서 내려오는것. 즉 0번 스테이지에서 연산이 되어진 것
		g_pDevice->SetTextureStageState( 1, D3DTSS_ALPHAOP, D3DTOP_DISABLE);  //D3DTOP_DISABLE : 알파값 쓰지 않겠다
//이상 위 코드에서 말하는 0번 또는 1번 스테이지 이런것들은 settexture할 때 입력하는 숫자의 텍스쳐를 말한다.
