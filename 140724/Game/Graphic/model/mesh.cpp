...
{
	if (m_attributes.empty())  //m_attributes : 속성값
	{
		const Matrix44 tm = m_localTM * m_aniTM * m_TM * parentTm;
		...
		if (!m_mtrls.empty())
			m_mtrls[ 0].Bind(shader);  //셰이더 상의 매트리얼 값을 설정(셋팅)
		if (!m_textures.empty())
			m_textures[ 0]->Bind(shader, "Tex");  //셰이더 상의 텍스쳐 값을 설정(셋팅)
	...
	else  //2개 이상의 매트리얼 적용시
	{
		...
		for (u_int i=0; i < m_attributes.size(); ++i)  //속성값 갯수만큼
		{
			const int mtrlId = m_attributes[ i].attribId;
			if ((int)m_mtrls.size() <= mtrlId)
				continue;
