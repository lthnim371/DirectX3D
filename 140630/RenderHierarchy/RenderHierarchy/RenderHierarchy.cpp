...
void Render(int timeDelta)
{
	...
	{  //wing1
		...
		g_wing.GetTM() = matS * matR * matT * g_mesh.GetTM();
	}
	...
	{  //wing2
		...
		//matS.SetScale( Vector3(0.7f, 0.1f, 0.3f) );
		Matrix44 matR;
		matR.SetRotationX( angle );
		Matrix44 matT;
		matT.SetTranslate( Vector3(180.f, 180.f, 0.f) );

		g_wing2.GetTM() = matS * matR * matT * g_wing.GetTM();  //공식대로라면 부모의 부모까지도 더해야 하지만 이미 g_wing의 TM에는 최상위 부모까지 더해진 상태이므로 g_wing2의 TM을 구할때 g_wing만 더해주면 되는것이다.

		//Tip : TM = SRT (크기 * 회전 * 이동 순으로 곱해줘야 한다.)
	}
