...
//3
MEMORYMONITOR_INNER_PROCESS();  //위젯 초기화
...
struct sGlobal : public memmonitor::Monitor<sGlobal, TYPE_NAME(sGlobal)>  //sGlobal가 상속 받도록 한다 
{
	...
	int nCmdShow)
{
	...
	//4
	//global를 메모리모니터에서 사용할거라서 그전에 초기화한거이다
	//INNER_PROCESS...OUTER_PROCESS도 있다.(이건 외부에서..접근??)
	...
	//테스트용2
	global->Array.push_back(1);
	global->Array.push_back(2);
	global->Array.push_back(3);
	global->Array.push_back(4);
