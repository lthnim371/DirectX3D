...
bool InitVertexBuffer()
{
	...
	D3DXCreateTextureFromFileA(g_pDevice, "../../media/강소라.dds", &global->texture);
...
void Render(int timeDelta)
{
		...
		)))
	{
		...
		g_pDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, true);  //알파블랜딩 on
		
		//어떠한 블랜딩 할건지
		g_pDevice->SetTextureStageState( 0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE );  //디퓨즈를 알파값으로 사용하겠다라는 의미  //텍스쳐도 가능하나 32비트 이미지일 경우 이미 알파값이 들어있다.
		g_pDevice->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_SELECTARG1 );
		g_pDevice->SetRenderState( D3DRS_SRCBLEND, D3DBLEND_SRCALPHA );  //원본
		g_pDevice->SetRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );  //데스티네이션
		...
		//출력 순서 중요 : z값(깊이 버퍼)에 상관없이 블랜딩을 적용시킬 물체는 기본 물체보다 나중에 그려져야 한다
		//즉, 당연한건 아니지만 쉽게 말하자면 나중에 그려지는것이 알파블랜딩이 적용된거임(알바블랜딩 적용시킬것을 나중에 그리자)
		// cube 출력.
